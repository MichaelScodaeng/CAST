...
from lib.gine import GINE
from lib.mlp import MLP
from lib.pe import PEARLPositionalEncoder, GetSampleAggregator
...
def construct_model(cfg: Schema, list_create_mlp, deg1=None, peptides=False, **kwargs): 
    ...
    if cfg.base_model == 'gine':
        base_model = GINEBaseModel(
            ...
            target_dim=target_dim, bn=cfg.gine_model_bn, pe_emb=cfg.pe_dims, peptides=peptides
        )
    else:
...
class PEARL_GNN_Model(nn.Module):
    ...
    def __init__(
        self, n_node_types: int, node_emb_dims: int, positional_encoding: nn.Module, base_model: nn.Module,
            pe_aggregate: str, feature_type: str = "discrete", peptides=False
    ) -> None:
        super().__init__()
        if peptides:
