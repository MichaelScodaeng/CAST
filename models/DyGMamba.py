...
# ... (FeedForwardNet and NIFEncoder are unchanged) ...
...
class DyGMamba(nn.Module):
    ...
    def get_features(self, node_interact_times: np.ndarray, padded_nodes_neighbor_ids: np.ndarray, padded_nodes_edge_ids: np.ndarray,
        ...
        if self.feature_preprocessor is not None:
            batch_size, seq_len = padded_nodes_neighbor_ids.shape
            node_ids_flat = padded_nodes_neighbor_ids.flatten()
            times_flat = padded_nodes_neighbor_times.flatten()
            raw_features_flat = self.node_raw_features[torch.from_numpy(node_ids_flat).long()]

            processed_features_flat = self.feature_preprocessor(node_ids_flat, times_flat, raw_features_flat)
            padded_nodes_neighbor_node_raw_features = processed_features_flat.view(batch_size, seq_len, -1)
        else:
