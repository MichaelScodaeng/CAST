...
class GraphMixer(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        raw_features = self.node_raw_features[torch.from_numpy(node_ids)]
        if self.feature_preprocessor is not None:
            # The "target" nodes for which we are computing embeddings
            processed_features = self.feature_preprocessor(node_ids, node_interact_times, raw_features)
            output_node_features = nodes_time_gap_neighbor_node_agg_features + processed_features
        else:
            output_node_features = nodes_time_gap_neighbor_node_agg_features + raw_features
        # --- MODIFICATION END ---
