# --- START OF FILE models/TCL.py (Modified) ---
...
class TCL(nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 time_feat_dim: int, num_layers: int = 2, num_heads: int = 2, num_depths: int = 20, 
        ...
        self.feature_preprocessor = feature_preprocessor # <<< STORE PREPROCESSOR
    ...
    def get_features(self, node_interact_times: np.ndarray, nodes_neighbor_ids: np.ndarray, nodes_edge_ids: np.ndarray,
        ...
        """
        # --- MODIFICATION START ---
        raw_features = self.node_raw_features[torch.from_numpy(nodes_neighbor_ids).long()]
        if self.feature_preprocessor is not None:
            # The structure is (batch, seq_len), so we can flatten and reshape
            batch_size, seq_len = nodes_neighbor_ids.shape
            node_ids_flat = nodes_neighbor_ids.flatten()
            times_flat = nodes_neighbor_times.flatten()
            raw_features_flat = raw_features.view(-1, raw_features.size(-1))

            processed_features_flat = self.feature_preprocessor(node_ids_flat, times_flat, raw_features_flat)
            nodes_neighbor_node_raw_features = processed_features_flat.view(batch_size, seq_len, -1)
        else:
            nodes_neighbor_node_raw_features = raw_features
        # --- MODIFICATION END ---
        
        # ... (rest of get_features is identical) ...
