...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
                 ...
                 dst_node_std_time_shift: float = 1.0, device: str = 'cpu',feature_preprocessor: Optional[nn.Module] = None): # <<< ADD NEW ARGUMENT):
        """
        ...
        if self.model_name == 'JODIE':
            ...
            self.embedding_module = GraphAttentionEmbedding(node_raw_features=self.node_raw_features,
                                                            ...
                                                            feature_preprocessor=feature_preprocessor) # <<< PASS PREPROCESSOR
        else:
            raise ValueError(f'Not implemented error for model_name {self.model_name}!')
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        if self.feature_preprocessor is not None:
            # The preprocessor generates the rich feature vector
            processed_features = self.feature_preprocessor(node_ids, node_interact_times, raw_features)
            # TGN/DyRep combine memory with features
            node_features = node_memories[torch.from_numpy(node_ids)] + processed_features
        else:
            # Original behavior
            node_features = node_memories[torch.from_numpy(node_ids)] + raw_features
